...
export function ItemEditor({ item }: { item: WardrobeItem }) {
  ...
  const [name, setName] = useState(item.user_facing_name ?? "");
  ...
  async function save() {
    ...
    const { error } = await supabase
      .from("wardrobe_items")
      .update({
        user_facing_name: name || null,
        category: category || null,
        color: color || null,
        pattern: pattern || null,
        occasion_tags: occasions,
        // Mark as user-confirmed; clears any "needs review" state.
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Rose floral top" />
      </div>
